...
public function update() {
    $this->validate();
    $query = "UPDATE $this->table_name SET name = ?, createdDate = ?, createdUser = ?, modDate = ?, modUser = ?, lockstate = ? WHERE category_id = ?";
    $stmt = $this->conn->prepare($query);
    $stmt->bind_param('sssssss', $this->name, $this->createdDate, $this->createdUser, $this->modDate, $this->modUser, $this->lockstate, $this->category_id);
    $stmt->execute();
    return $stmt->affected_rows;
}
